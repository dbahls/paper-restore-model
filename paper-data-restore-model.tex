...
% TODO: wir sollten noch die Terminologie klï¿½ren. wie bezeichnen wir 
%
% * das Prinzip, Werte zu referenzieren:  Bisher nannte ich es Reference Model
%
% * so ein Daten-Template, das IDs anstelle von Werten enthï¿½lt: data reference template? ddocks template?
%
%
% Solange das noch nicht so ganz gefunden ist, kï¿½nnen wir einfach diese Tokens verwenden:
%
\newcommand{\refmodel}{
...
\title{
%On the Reproducibility of Statistical Research
%On the Reproducibility in Computational Statistics
...
These circumstances give us reason to pursue a data restore model based on fine-grained referencing that allows to trace data provenance to the original archive that is in charge of curation.
One goal is to enable data publication in difficult settings.
%BZ: "this research" koennte verwechselt werden mit der research, die reproduzierbar sein soll. wie waers mit "our goal is to enable ..."
...
On this basis, we develop the requirements for a data restore model and elaborate a generic design and its implication on an overall data management infrastructure.
We further present a proof of concept implementation on the basis of a real-world example from economics.
Eventually we close with the vision of a data and code ontology that carries empirical models from paper to a re-usable semantic level.
...
%This is very often not the case in computational research 
...
%One of them is the plain fact that the data on which the research is built often remains unavailable to others which gives rise for a more organized data management practice. \cite{Wood1298248}
...
%A comprehensive review of the JMCB\footnote{Journal of Money, Credit, and Banking} data+code archive reveals that results cannot be reproduced even when these resources are given.  
...
%Such data sets can be transformed into anonymized public-use files in general which needs verification in terms of cell sizes and de-anonymization and requires additional efforts, often on the researchers' side.
...
%We aim to design this referencing technique in a way that allows for seamless integration with preferred research tools and source code, so that no further manual work is required for setting up data and code resources during replication attempts.
%The restore model is designed to seamlessly integrate with preferred research tools and source code, so that no further manual work is required for setting up data and code resources during replication attempts.
...
%In this paper we aim to overcome these challenges for the domain of statistical data and elaborate on a holistic solution for an infrastructure that enables reconstruction of individual research data sets. 
%This infrastructure is basically realized by an underlying data reconstruction model, which builds the bridge between the researchers' preferred software tool and the research data. 
%The approach follows the idea of a data reconstruction model by which we develop the requirements it carries on an infrastructure.
%We present an implementation of the model that tries to seamlessly integrate with any software tool preferred by a researcher.
%By utilizing the model generic ids are assigned to all data values regardless of their original format and their physical location.
%This allows for reconstructing and referencing the data set out of the researchers programming tool.
...
%BZ: veraltet. muss man am schluss aktualisieren, wenn das paper steht
...
%Subsequently, we elaborate a generic design and outline the big picture of the overall data mangement infrastructure and workflows.
...
In case the researcher used own data, the data would have to be submitted to the dedicated archive first where the respective data template and its IDs involved are generated first.
...
%For a more detailed view on the implementation, please take a look at the source code.\footnote{\url{https://github.com/dbahls/ddocks} \url{}}
...
%This allows us to comfortably use the code within the source code of a research project from the begin
%
%
%\begin{quote}
...
%BZ: die zusammenlegung finde ich sehr sinnvoll


%
% Metadaten, DDI, SDMX, LOD etc. können wir im Prinzip als Technologien zur Dokumentation und zur Unterstützung von Data Retrieval betrachten.
% Wir suchen aber ja nicht nach Daten sondern haben konkrete Pointer. Wsa anderes wird in der Reproduktionsfrage auch nicht benötigt.
% Auskehren, dass unsere Technik sich prima mit Sem Web Technologien integriert. 
% Die Diskussion um geeignete Metadaten und Ontologien sollte vielleicht unabhängig von unserer Problemstellung geführt werden?
%
% Andererseits geht es auch um die Integration des Modells mit der Literatur. 
% Irgendwie wollen wir ja schon mit reinbringen, dass Template und Source Code dann auch mit einem Artikel assoziiert werden sollte.
% Dafür brauchen wir eine Ontologie.

% Das hängt auch mit der möglichen Anwendung zusammen, also
%  * Zielgruppe Studenten, Doktoranden, Wissenschafter, als Lehrmaterial allgemein oder für effizientere Forschung
%  * Review-Prozesse
%
% So eine Ontologie könnte es erlauben Daten und Code so zu verweben, dass man gut anbieten kann anstelle der geschützten Daten einen offenen Zwischendatensatz zu verwenden, mit dem weitergerechnet werden kann oder so....
%


%
% Input für eine Data and Code Ontologie
%


% Future work will be on that ontology too (name "data and code ontology"?)
% app scenario, for students, researchers, reviewers
% responsibility of libraries to catalog these items (restore model instances, instances of that ontology)
% archives should pick up the resources and apply restore model, if not ours then another one that is as effective

% dabei berücksichtigen wir bereits aufgestellte anforderungen

%
% Anforderungen aus Economics und Social Sciences zur Verbesserung der Reproduzierbarkeit
%
% Code ist immer erwähnt. und sie sagen, wie die readme datei aussehen muss usw.
% Diese ganzen elemente können in einer ontologie organisiert werden 
%

there are other recommendations in \cite{kleiber2012reproducible}   % replication files, code, technical information
...
% habs nicht gelesen, geht aber direkt mit reproduzierbarkeit los. muessen mal sehen was die da an guidelines reinbringen, deckt sich bestimmt mit den anderen
...
% db: ich bin da nicht wirklich skeptisch, aber die frage muss schon mal rein
% db: naja, das ist schon ganz schön heavy, was da an ID-Mengen entsteht. Wir sollten einfach einräumen, dass wir auf die Frage noch keine Antwort haben
%However the scalability question also applies for all the other RDF approaches in OpenGov, LOD etc, see billion triple challenge
...
% The final version of the paper will be using Sweave, for which the sources will be available as well. \footnote{\url{https://github.com/dbahls/paper-src}}
% db: die müllige revision history braucht man ja nicht in die welt schreien  ; )
% We used Sweave for this publication ourselves and invite you to make your own thoughts
